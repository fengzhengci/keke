...
# 通过pidof命令判断进程是否纯在
def ckpid(s):
    result, output = subprocess.getstatusoutput('/sbin/pidof %s' % s.get('name'))
    cklog(s)
    if result:
        s['status'] = "FAILED"
    else:
...
# def ckjavapid(s):
#     javadir = '/data/linkdood/im/IMServer/'
...
# 通过端口判断进程是否存在
def ckport(s):
    result, output = subprocess.getstatusoutput("/usr/sbin/ss -tnl | /bin/awk '$4~/.*:%s/{print $4}'" % s.get('port'))
    cklog(s)
    if output:
        s['status'] = "SUCCESS"
    else:
...
def vrvcheck():
    ...
    for server in vrvservices.services.keys():
        for s in vrvservices.services.get(server):
            if server == 'cpp' or (server == 'base' and s.get('name') in (
                    'mysqld', 'turnserver', 'nginx', 'prelogin', 'upload', 'cloud', 'ddio', 'apnsagent',
                    'sharecomment', 'miniweb', 'oauth', 'minidood', 'organization', 'favorite', 'mail')):
    ...
    for t in th_list:
        t.join()
    logger.debug(vrvservices.services)
...
def vrvctrl(name, ctrl):
    ...
    if name == "cj":
        ...
        ctrl = 'fstop'
    cmd = '/usr/bin/linkd %s %s' % (name, ctrl)
    logger.info(cmd)
